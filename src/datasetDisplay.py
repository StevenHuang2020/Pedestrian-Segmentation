...
from modules.file.file import pathsFiles
...
def scatterData(data):
    ...
    plotHist(data[:, 0], ax, label='height range')
    plt.show()
...
def plotData(data):
    ...
    label = 'height range'
    plot(data[:, 0], ax, label=label)
    ax = plt.subplot(2, 2, 2)
    plotHist(data[:, 0], ax, label=label)

    label = 'weight range'
    ax = plt.subplot(2, 2, 3)
    plot(data[:, 1], ax, label=label)
    ax = plt.subplot(2, 2, 4)
    plotHist(data[:, 1], ax, label=label)
    plt.show()
